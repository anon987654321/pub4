...
module Deploy
  module GateEnvironment
    ...
    INTEGRITY_GATES = [
      ...
      Gate.new(name: "relayd_smoke", path: "OPERATOR/openbsd/deploy_smoke_gate.rb", needs: %i[repo]),
      ...
      Gate.new(name: "vps_health", path: "OPERATOR/openbsd/health_check.rb", args: ["--core"], needs: %i[vps live_http], optional: false),
    ].freeze
    ...
    def post_pull_warning
      ...
      <<~WARN
        ...
        integrity: note — run: zsh OPERATOR/openbsd/sh/vps_ci.sh <app>  (serial, one app at a time)
