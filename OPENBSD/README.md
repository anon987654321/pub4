...
Start here. `RUNBOOK.md` is the single operational companion for live VPS work; read it before SSH, `doas`, deploy, DNS, relayd, NSD or recovery. `OPENBSD/bin/check` is the local gate and `OPENBSD/bin/check-vps` checks vm23. `OPENBSD/data/operator.yml` is the command/recipe source; do not create another command catalogue.
