...
class CiGuardTest < Minitest::Test
  ...
  def test_lock_path_defaults_to_the_shared_root_owned_file
    with_env("PUB4_CI_LOCK" => nil) do
      assert_equal "/var/db/pub4/ci.lock", Operator::CiGuard.lock_path
      assert_equal "/var/db/pub4/ci.lock.holder", Operator::CiGuard.holder_path
    end
  end
  ...
  def test_override_is_honoured_in_a_private_directory
    Dir.mktmpdir do |dir|
      ...
      with_env("PUB4_CI_LOCK" => path) do
        assert_equal path, Operator::CiGuard.lock_path
      end
    end
  end
  ...
  def test_override_is_refused_from_a_world_writable_directory
    Dir.mktmpdir do |dir|
      ...
      with_env("PUB4_CI_LOCK" => path) do
        capture_stderr { assert_equal "/var/db/pub4/ci.lock", Operator::CiGuard.lock_path }
      end
    end
  end
  ...
  def test_lock_file_is_not_made_world_writable
    in_private_lock do |path|
      Operator::CiGuard.with_lock { nil }
  ...
  def test_with_lock_yields_and_returns_the_block_value
    in_private_lock do
      assert_equal :yielded, Operator::CiGuard.with_lock { :yielded }
    end
  end
  ...
  def test_holder_file_exists_while_the_lock_is_held_and_not_after
    in_private_lock do |path|
      ...
      Operator::CiGuard.with_lock { during = File.exist?(holder) }
  ...
  def test_with_lock_reports_eacces_on_open_instead_of_crashing
    in_private_lock do |path|
      ...
      begin
        error = nil
        capture_stderr { error = assert_raises(SystemExit) { Operator::CiGuard.with_lock { :yielded } } }
  ...
  def test_safe_read_falls_back_to_unknown_on_eacces
    ...
    begin
      assert_equal "unknown", Operator::CiGuard.safe_read(__FILE__)
    ensure
  ...
  def test_unreadable_load_warns_rather_than_reporting_zero
    ...
    with_unreadable_load do
      err = capture_stderr { assert_in_delta 0.0, Operator::CiGuard.current_load }
    end
  ...
  def test_load_under_the_limit_returns_without_waiting
    with_load(1.0) do
      slept = record_sleeps { Operator::CiGuard.check_load! }
  ...
  def test_load_over_the_limit_waits_and_proceeds_when_it_falls
    ...
    with_load_sequence(readings) do
      slept = nil
      err = capture_stderr { slept = record_sleeps { Operator::CiGuard.check_load! } }
  ...
  def test_load_that_never_falls_gives_up_rather_than_waiting_forever
    with_env("PUB4_CI_LOAD_WAIT" => "0") do
      with_load(9.0) do
        ...
        err = capture_stderr { code = assert_raises(SystemExit) { Operator::CiGuard.check_load! } }
  ...
  def stub_singleton(name, &body)
    meta = Operator::CiGuard.singleton_class
    meta.send(:alias_method, :"__real_#{name}", name) if Operator::CiGuard.respond_to?(name)
    Operator::CiGuard.define_singleton_method(name, &body)
  end
  ...
  def with_unreadable_load
    sc = Operator::LoadAverage.singleton_class
