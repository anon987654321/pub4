...
# CI-facing wrapper around DesignTokens.dialect_token_drift (RAILS/design_tokens.rb)
...
# Fails CI on drift; fix with `ruby RAILS/scripts/sync_dialect_tokens.rb`.
...
if findings.empty?
  ...
  findings.each { |f| warn "dialect_token_drift_check: #{f} -- run: ruby RAILS/scripts/sync_dialect_tokens.rb" }
  exit 1
end
