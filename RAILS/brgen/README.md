...
`Marketplace::*` owns the seller, listing, offer, checkout and order semantics. Dintero owns marketplace payment authorization, capture and split allocation; seller payout destinations are accepted only when Dintero reports `ACTIVE`. Solidus is an optional commerce kernel for catalog/cart/fulfillment experiments, not the seller ledger.

Solidus 4.7 is the current upstream guide version and its marketplace offering now includes mixed carts, split shipments, merchant storefronts and managed payouts. The existing `solidus_marketplace` 0.1.0 extension is deliberately not a dependency. citeturn242644search0turn242644search1
