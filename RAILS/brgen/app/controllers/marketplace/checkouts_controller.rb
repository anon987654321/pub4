...
class Marketplace::CheckoutsController < Marketplace::BaseController
  ...
  def create
    ...
    unless order
      redirect_to marketplace_cart_path, alert: "Cart has no payable items"
      return
    end
    ...
    url =
      ...
      when "stripe"
        Marketplace::Payments::StripeCheckout.start!(
          order: order,
          success_url: marketplace_checkout_url(provider: "stripe", order_id: order.id),
          cancel_url: marketplace_cart_url
        )
      when "vipps"
        Marketplace::Payments::VippsCheckout.start!(
          order: order,
          return_url: marketplace_checkout_url(provider: "vipps", order_id: order.id)
        )
      else
        redirect_to marketplace_cart_path, alert: "Unknown payment provider"
        return
      end
    redirect_to url, allow_other_host: true
  rescue Marketplace::Payments::NotConfigured => e
    redirect_to marketplace_cart_path, alert: e.message
  rescue StandardError => e
    Ground::Swallow.log(e, context: "Marketplace::CheckoutsController#create") if defined?(Ground::Swallow)
    redirect_to marketplace_cart_path, alert: "Checkout failed: #{e.message}"
  end
  ...
  def show
    ...
    if order&.payment_status == "pending" && params[:provider].present?
      ...
      redirect_to marketplace_order_path(order), notice: "Payment #{order.payment_status} via #{order.payment_provider || params[:provider]}"
    else
