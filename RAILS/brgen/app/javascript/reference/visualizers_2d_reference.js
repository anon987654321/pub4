...
// shared/frontend/layouts/visualizer.js, which bound to a #canvas no view ever
// rendered. Nothing loaded it, so none of them ever ran, and it was being
// compiled into brgen and amber and shipped dead. It was deleted in 248e23795
// on exactly that reasoning.
//
// The reasoning was right about the file and wrong about the contents: seven
// named visualisers are not dead weight just because their host was never
// mounted. This is the recovered source, kept so the port to WebGL has
// something to port FROM.
//
// They are canvas-2D, writing pixels into an ImageData buffer. The live tunnel
// is WebGL and a canvas carries one context, so these cannot be revived as-is
// beside it. The port is to the same gl.POINTS form the tunnel uses: one
// shader, a position function per visualiser, which is what all seven already
// are underneath the rasteriser.
