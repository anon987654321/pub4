...
- **Honesty:** photo polish ≠ ML segment/matting; fingerprint ≠ embedding; tips = rules; live_streams = style sessions (no video).
- **AI:** `OPENROUTER_API_KEY` for LLM; otherwise heuristics. MASTER photos only if `AMBER_ENABLE_MASTER_PHOTO=1`.
- **Hygiene:** `DeclutterHygieneJob` daily (challenges + 30d box); see `config/recurring.yml`.
