...
# amber routed and controllered the social stack without ever creating its tables, so /notifications answered 500.
# The engine carries the same DDL, but an engine ships migrations it cannot install; brgen wrote its own 20260524113000.
# follows stays out: amber's is keyed follower_id/followee_id, not the engine's polymorphic followable.
class CreateAmberSocialTables < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :reactions, %i[user_id reactable_type reactable_id kind],
              unique: true, name: "idx_reactions_unique_user_target_kind"
