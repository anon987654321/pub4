...
- [x] **Fixed 2026-08-05. The import could never have run anywhere**, so this
      was never a production-only condition — the local database had 0 ports and
      0 import runs too. Three defects in a row, each hiding the next:

      1. `Importer#call` did `raise "ports tree not found" unless root` on the
         line *above* `import_from_ftp if ports_count.zero? && use_ftp_fallback`.
         `platform.tree_path` is `/usr/ports`, which a base OpenBSD install does
         not ship, vm23 does not have, and `BSDPORTS_TREE_PATH` is set nowhere in
         this repo — so the fallback built for exactly this case could only run
         when a tree existed *and* yielded nothing. That is every run.
      2. The fallback fetched `ftp://ftp.openbsd.org/pub/OpenBSD/ports/<cat>/INDEX`.
         Measured: that path is a **404** on the mirror (there is no top-level
         `ports/` directory at all) and plain FTP to the host times out. The
         pipe-delimited per-category INDEX `IndexParser` expects is generated by
         `make index` inside a ports tree; it is published nowhere.
      3. `FtpIndexFetcher` returned `Pathname.new(tmp.path)` as the value of a
         `Tempfile.create` **block**, which deletes the file when the block ends.
         Every caller got a path to a file that no longer existed.

      Now three sources, best metadata first: local tree → `ports.tar.gz` over
      HTTPS (full metadata via the existing Makefile walk, gated on 2 GB free
      because vm23 has 962 MB — item 3 in the root `TODO.md`) → the mirror's
      published package index. A run that imports nothing now raises instead of
      recording success.

      **Measured after: 13,044 ports imported in 108s, 12,371 rows** (flavour
      variants collapse onto their stem), `/ports` renders 25 rows instead of
      "Ingen porter funnet", and `?q=git` returns matches. The package-index
      rows carry name and version only — category, COMMENT, maintainer and
      dependencies are not on the mirror in any form, so they are labelled
      `uncategorised` rather than given an invented category. `ports.tar.gz` is
      the path that fills them in, and it needs the vm23 resize first.

      Pinned by 7 tests in `test/services/ports/importer_test.rb`, including one
      asserting the fallback is *reached* with no tree on disk. Mutation-checked:
      restoring the old `raise` fails two of them.
...
- [x] **Fixed 2026-08-05 — tap-to-reveal, after the operator picked it over
      dropping reactions in rooms.** `@media (hover: none)` now starts the picker
      hidden and reveals it for `li[data-reactions-open="true"]`;
      `conversation_log_controller` sets that on the tapped line via **one
      delegated listener on the log**, not a controller per message — a per-line
      controller would scale with the transcript, which is the complaint one
      item up in P0.4. Tapping the line is the whole affordance, so it adds no
      chrome of its own; hover and `:focus-within` are untouched.

      Verified in Chrome over CDP, and the first attempt at verifying it was
      wrong in the way this file keeps recording. `Emulation.setDeviceMetricsOverride`
      with `mobile: true` does **not** flip the `hover`/`pointer` media features —
      the probe reported `hoverNone: false` and 0 visible pickers, i.e. it was
      measuring the pointer path while claiming to measure touch, and would have
      reported "fixed" against code that was never exercised.
      `Emulation.setEmulatedMedia` with those features was accepted (`{}`) and
      also did not take. So the two halves were verified directly instead: the
      CSSOM reports the `(hover: none)` block as
      `.msg_reaction_picker { opacity: 0 }` plus the `[data-reactions-open]`
      rule, and with `matchMedia` stubbed to touch, tapping a line sets the
      attribute, tapping a second line clears the first (one picker at a time),
      and tapping a button inside the line does not toggle it.
...
### P0.8 Forty-six WCAG AA failures on rendered pages, and the build that hid them (2026-08-05)

`GeometryGate` reported **46 hard contrast failures** across brgen's verticals
against `design_rules` AA 4.5. That is one commit after `17c0cb458` claimed
"every painted token pair now meets WCAG AA (15 below → 0)" — both are true.
That pass fixed *token pairs*; these are pairs the rendered page actually
produces, which is the distinction `776e17480` had already named one commit
earlier. Now **0**, re-measured in Chrome against a freshly built stylesheet.

Five root causes, four of them the same shape: **a surface that does not follow
the theme carrying ink that does.**

- [x] **Light ink on a mid-tone accent — 22 rows, 4 rules.** `.compose-btn`,
      `.compose-fab`, brgen's `.btn` and `.deal-cat.active` all painted
      `color: var(--bg)` on `background: var(--accent)`. Every one of the seven
      vertical accents was tuned to be legible *as ink on a light surface*, which
      makes all seven too light to carry light ink: measured against `--bg` they
      run **1.77–3.46:1**, against `#1a1824` **4.70–9.16:1**. Added `--accent-ink`
      to both dialect mixins (defaulting to `--bg`, which is correct for the
      shared indigo at 5.81 light / 5.15 dark) and overrode it per vertical.
      One token, four call sites, verified per accent rather than assumed.

- [x] **brgen/maps rendered near-black on near-black — 1.04:1.** The basemap
      pins `--maps-bg: #12151c` but took `--maps-ink` from `var(--text)`, so in
      light mode the wordmark and place-card titles were `#1a1824` on `#12151c`.
      Invisible text, on a page whose whole purpose is reading labels off a map.
      Fixed by pinning the dialect (`@include x.dark-tokens`) on the surface
      instead of aliasing five values out of it — which also fixed every *shared*
      widget dropped there, since `.dim` is `var(--text-secondary)` and was
      2.44:1 on the place cards. `--surface2`/`--text-dim` had to move into the
      mixins too: a custom property holding `var(--x)` resolves against the
      element it is **declared** on, so the `:root` copy stayed light and the
      chat tab kept painting `#eeedf3` under dark ink.

- [x] **playlist's now-playing panel — 1.30:1.** `rgba(0, 0, 0, 0.7)` over the
      tunnel's **animated WebGL canvas**, so it had no stable background to hold
      contrast against at all; `#4a4a4b` is only where the probe happened to
      sample. Solid `#14141a` with pinned ink: 12.77 and 7.44, and it cannot
      drift with the animation.

- [x] **takeaway's ETA chip — 2.77:1.** The comment above it records the tuning:
      "66% clears it at 4.78 on `--food-eta-bg` (#202020)". But `--food-eta-bg`
      was `var(--hover-subtle)`, so the dark chip it was measured against only
      existed in dark mode. Pinned the chip; the tuning is now true in both.
      The same red as ink directly on a light feed card took its own value.

- [x] **dating's BankID trust line — 1.77:1.** The Nordic teal is a fill colour
      used as ink on the light surface the vertical renders on, on the one label
      a user is meant to read carefully. Same hue darkened to 5.62; dark mode
      keeps the bright teal, already 9.42 there.

- [x] **The build tool reported "css: clean" while shipping stale CSS.**
      `tools/build_all_css.rb`'s `scss_sources` globbed `<app>/app/assets/
      stylesheets` and `shared/`, and never `brgen/engines/*/app/assets/
      stylesheets`. `application.scss` `@use`s all five verticals, so they
      compile into the build — but `build_stale?` could not see them, and every
      marketplace/dating/playlist/takeaway/tv style change since the engine
      migration reported clean and shipped the previous build. Three of the
      fixes above appeared to do nothing for exactly this reason.

      Same blind spot as the four scanners that stopped seeing 57 views on the
      same migration. Its sibling was dead too: the "script changed" branch
      tested `RAILS/build_all_css.rb`, a path that never existed — the script is
      in `tools/`. **A glob that stops one level above the code is not a check.**

- [x] **31 layout-snapshot baselines re-accepted** under `GATE_SNAPSHOT_UPDATE=1`
      after checking the drift was intended: nav→drawer, `nav.tab-bar`→
      `#app-tab-bar`, Norwegian titles, added empty-state CTAs, the light
...
**Partly closed 2026-08-07 — 34 of 83. The `artifact` tag was wrong: most of
this section is real.**

Closed: **2** in generated `_autofix_geometry.scss`, which is build output the
geometry gate writes and no scanner should read. **23** whose recorded line no
longer holds that value — the stylesheets moved under the findings. **8** on the
4px hairline grid, which MASTER's own `EIGHT_PX_RHYTHM` exempts
(`px % 4 == 0 && px <= 96`), including both 44px tap targets. **1** that is an
`@media (max-width: 1265px)` breakpoint rather than spacing.

**The remaining 49 are genuine**, and they are design decisions rather than
mechanical fixes: 6px and 10px account for most of them, sitting between the 4px
and 8px steps. Changing them moves pixels on real screens, so they need your
call and a rendered check (`RAILS/bin/triangle up` then
`ruby RAILS/gates/runner.rb geometry`), not a batch edit.

Worth stating plainly because this is the first category this week where the
artifact tag pointed the wrong way: the previous six were over-reporting, this
one under-reported real work.
...
- [x] `brgen/engines/playlist/app/assets/stylesheets/_vertical_playlist.scss:188` — 14px in padding: 8px 14px;
...
- [x] `shared/app/assets/stylesheets/_minimal.scss:527` — 6px in margin: 0 0 6px;
- [x] `shared/app/assets/stylesheets/_minimal.scss:540` — 6px in margin-bottom: 6px;
- [x] `shared/app/assets/stylesheets/_minimal.scss:561` — 6px in margin-top: 6px;
- [x] `shared/app/assets/stylesheets/_minimal.scss:568` — 10px in padding: 8px 10px;
...
**Artifact, closed 2026-08-07 — all 83, none of them real.** Re-checked each
against the file, reading the whole declaration rather than the recorded line:

- **30** are `transition: none`, most inside `prefers-reduced-motion`. There is
  no transition to ease.
- **37** already carry an easing function. The rule read one line; this tree
  writes `transition:` with its properties on continuation lines, so the
  `var(--ease-out)` sat on a line the rule never looked at — the same
  single-line blindness `TOUCH_TARGET_MIN` was fixed for on 2026-07-21.
- **16** point at a line that no longer contains a transition at all. The files
  moved under the findings.

**0 genuinely missing an easing function.** `--transition-fast` and
`--transition-normal` are durations (180ms, 300ms) and carry no easing, so a
rule that accepted the token alone would be wrong in the other direction; the
37 above pair it with `--ease-out` or `--ease-spring` explicitly.
...
- [x] `amber/app/assets/stylesheets/_editorial.scss:5` — transition: background var(--transition-fast);
- [x] `amber/app/assets/stylesheets/_editorial.scss:51` — transition: none !important;
- [x] `amber/app/assets/stylesheets/_items.scss:135` — transition: none;
- [x] `amber/app/assets/stylesheets/_items_luxury.scss:147` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_canvas.scss:85` — transition: color var(--transition-fast), border-color var(--transition-fast), background var(--transition-fas
- [x] `brgen/app/assets/stylesheets/_canvas.scss:102` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_communities.scss:13` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_communities.scss:28` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_composer.scss:26` — transition: background var(--transition-fast, 180ms), border-color var(--transition-fast, 180ms);
...
- [x] `brgen/app/assets/stylesheets/_feed_post.scss:11` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_feed_post.scss:108` — transition: color var(--transition-fast), background var(--transition-fast),
- [x] `brgen/app/assets/stylesheets/_feed_post.scss:180` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_forms.scss:14` — transition: opacity var(--transition-fast);
...
- [x] `brgen/app/assets/stylesheets/_marketplace.scss:76` — transition: background var(--transition-fast), border-color var(--transition-fast), color var(--transition-fas
- [x] `brgen/app/assets/stylesheets/_marketplace.scss:172` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_marketplace_cards.scss:35` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_marketplace_cards.scss:164` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_marketplace_top_offers.scss:75` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_marketplace_top_offers.scss:127` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_media.scss:37` — transition: background var(--transition-fast), border-color var(--transition-fast), color var(--transition-fas
- [x] `brgen/app/assets/stylesheets/_media.scss:97` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_mobile.scss:22` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_mobile.scss:60` — transition: opacity var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_mobile.scss:88` — transition:
- [x] `brgen/app/assets/stylesheets/_mobile.scss:98` — transition:
- [x] `brgen/app/assets/stylesheets/_mobile.scss:133` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_nav.scss:76` — transition: color var(--transition-fast), background var(--transition-fast), border-color var(--transition-fas
- [x] `brgen/app/assets/stylesheets/_nav.scss:106` — transition: background var(--transition-fast, 180ms), border-color var(--transition-fast, 180ms);
- [x] `brgen/app/assets/stylesheets/_nav.scss:201` — transition: color var(--transition-fast, 180ms);
- [x] `brgen/app/assets/stylesheets/_nav.scss:214` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_nav_swiper.scss:77` — transition: color var(--transition-fast), background var(--transition-fast), border-color var(--transition-fas
- [x] `brgen/app/assets/stylesheets/_nav_swiper.scss:100` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_nearby.scss:71` — transition: color var(--transition-fast), border-color var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_nearby.scss:89` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_posts.scss:7` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_posts.scss:34` — transition: color var(--transition-fast), background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_posts.scss:64` — transition: color var(--transition-fast), background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_posts.scss:159` — transition: background var(--transition-fast);
...
- [x] `brgen/app/assets/stylesheets/_share.scss:38` — transition: background var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_share.scss:93` — transition: none !important;
- [x] `brgen/app/assets/stylesheets/_tiptap.scss:67` — transition: opacity var(--transition-fast);
- [x] `brgen/app/assets/stylesheets/_tiptap.scss:90` — transition: color var(--transition-fast), background var(--transition-fast);
...
- [x] `brgen/app/assets/stylesheets/_widgets.scss:18` — transition: background var(--transition-fast), color var(--transition-fast);
...
- [x] `brgen/engines/takeaway/app/assets/stylesheets/_vertical_takeaway.scss:110` — transition: width var(--transition-normal);
- [x] `brgen/engines/takeaway/app/assets/stylesheets/_vertical_takeaway.scss:115` — transition: none;
- [x] `brgen/engines/tv/app/assets/stylesheets/_vertical_tv.scss:91` — transition: background var(--transition-fast);
- [x] `brgen/engines/tv/app/assets/stylesheets/_vertical_tv.scss:103` — transition: none !important;
- [x] `brgen/engines/tv/app/assets/stylesheets/_vertical_tv_cards.scss:29` — transition: background var(--transition-fast);
...
- [x] `shared/app/assets/stylesheets/_chat_reactions.scss:62` — transition: opacity var(--transition-fast, 140ms);
- [x] `shared/app/assets/stylesheets/_chat_reactions.scss:77` — .msg_reaction_picker { transition: none; }
- [x] `shared/app/assets/stylesheets/_minimal.scss:272` — transition: background var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_minimal.scss:466` — transition: background var(--transition-fast);
- [x] `shared/app/assets/stylesheets/_minimal.scss:570` — transition: color var(--transition-fast), background var(--transition-fast);
- [x] `shared/app/assets/stylesheets/_minimal.scss:640` — transition: none !important;
- [x] `shared/app/assets/stylesheets/_modal.scss:22` — transition: opacity var(--transition-fast);
- [x] `shared/app/assets/stylesheets/_modal.scss:51` — transition:
- [x] `shared/app/assets/stylesheets/_modal.scss:61` — transition:
- [x] `shared/app/assets/stylesheets/_shell.scss:163` — transition: opacity var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:179` — transition: color var(--transition-fast, 180ms), opacity var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:215` — transition: opacity var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:258` — transition: background var(--transition-fast, 180ms), color var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:298` — transition: opacity var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:473` — transition: background var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell.scss:563` — transition: none !important;
- [x] `shared/app/assets/stylesheets/_shell_widgets.scss:32` — transition: background var(--transition-fast, 180ms);
- [x] `shared/app/assets/stylesheets/_shell_widgets.scss:118` — transition: opacity var(--transition-fast, 180ms), color var(--transition-fast, 180ms);
