    ...
    class Introspection
      class << self
        ...
        def run_council_review
          ...
          key_files = %w[master.rb pipeline.rb stages.rb llm.rb chamber.rb executor.rb commands.rb enforcement.rb
                         introspection.rb]
          ...
          if result.ok?
            ...
            rating_match = response.match(%r{(\d+)\s*/\s*10|rating[:\s]+(\d+)}i)
        ...
        def check_exception_handling(content, file)
          ...
          issues << "#{file}: Bare rescue found" if content.match?(/rescue\s*$/) && !BARE_RESCUE_ALLOWED.include?(file)
          issues
        end
        ...
        def check_logic_patterns(content, file)
          ...
          issues << "#{file}: Mixed symbol/string hash access" if symbol_keys > 5 && string_keys > 5
          issues
        end
