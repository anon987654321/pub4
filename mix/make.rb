...
# Synthesize a J Dilla beat from primitives (no source needed):
#   ruby make.rb dilla [out.mp3]               86 BPM × 8 bars, lo-fi master
#
...
# ── dilla ─────────────────────────────────────────────────────────────────────
# Synthesize an MPC-style Dilla beat from primitives. No source file.
# Kick = sine sweep 100→40Hz with exp env. Sub = 32.7Hz sine following kick.
# Snare/hat/open = white-noise carrier gated by envelope sums + EQ.
# Crackle, tape sat, lo-fi lowpass, soft limit. 86 BPM, 8 bars.

DILLA_BPM  = 86
DILLA_BARS = 8

def dilla(dest = File.join(DIR, "dilla.mp3"))
  bpm  = DILLA_BPM
  bars = DILLA_BARS
  beat = 60.0 / bpm
  bar  = beat * 4
  ...
  render "dilla beat (#{bpm} BPM × #{bars} bars → #{total}s)", dest,
    inputs: inputs, map: "[out]", args: ["-b:a", "320k"], filter: filt
  puts "dilla -> #{dest}"
end
...
when "dilla"
  dilla(ARGV[1] || File.join(DIR, "dilla.mp3"))
when nil, /\Av\d+\z/
  ver = ARGV[0] || "v11"
  ...
  abort "usage: ruby make.rb [v7|v8|v9|v10|v11]   ruby make.rb demux <url-or-path> [deep]   ruby make.rb dilla [out.mp3]"
end
