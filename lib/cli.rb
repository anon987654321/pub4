...
module MASTER
  class CLI
    ...
    def handle(input)
      ...
      when 'review'
        review_with_crew(arg)

      when 'workflow', 'wf'
        execute_workflow(arg)

      when 'optimize'
        arg == 'costs' ? optimize_costs : "Unknown optimize command. Use 'optimize costs'."
    ...
    def help_text
      <<~HELP
        Commands:
          ...
          web <url>      Browse URL
          workflow "<task>" Execute natural language workflow
          optimize costs Analyze and optimize LLM costs
          exit           Quit
      HELP
    end
    ...
    def review_with_crew(path)
      return 'Usage: review <path>' unless path
      ...
      results = crew.review(code, full) do |agent_name, status, count|
        case status
        when :started
          broadcast "  ▶ #{agent_name}: analyzing..."
        when :completed
          broadcast "  ✓ #{agent_name}: found #{count} findings"
        end
      end
      ...
      output << "By severity: #{summary[:by_severity].inspect}"
      output << "Duration: #{sprintf('%.2f', summary[:total_duration])}s"
      output << ""
    ...
    def execute_workflow(task_description)
      return 'Usage: workflow "<task description>"' if task_description.nil? || task_description.empty?
      ...
      output = []
      output << "=" * 60
      output << (results[:success] ? "✓ Workflow completed successfully" : "✗ Workflow failed")
      output << "=" * 60
      output.join("\n")
    end
    ...
    def optimize_costs
      ...
      output << "=" * 60
      output << "Current session cost: #{llm.cost_summary if llm.respond_to?(:cost_summary)}"
      output << ""
      ...
      if results[:success]
        ...
        if applied&.any?
          ...
          output << "✓ Applied #{applied.size} optimizations"
          output << "  Projected savings: $#{sprintf('%.4f', total_savings)}"
        else
          output << "No optimizations needed at this time"
        end
      else
