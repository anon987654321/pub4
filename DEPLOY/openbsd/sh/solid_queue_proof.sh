...
su -m "$app" -c "export HOME=/home/${app}; cd ${app_dir} && bundle34 exec rails runner -e production \"n = SolidQueue::Process.count; puts \\\"solid_queue: ${app} processes=\\\${n}\\\"; exit(n.positive? ? 0 : 1)\""