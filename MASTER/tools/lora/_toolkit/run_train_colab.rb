...
def dataset_cell(options)
  ...
  <<~PYTHON.strip
    ...
    target = "/content/pub4/MASTER/tools/lora/#{SUBJECT}/dataset"
...
def render_cell(options)
  <<~PYTHON.strip
    ...
    proc = subprocess.Popen(
        ["ruby", "/content/pub4/MASTER/tools/lora/_toolkit/colab_session.rb", "#{SUBJECT}"],
...
def markdown(options)
  ...
  <<~MARKDOWN
    ...
    Generated by `MASTER/tools/lora/_toolkit/run_train_colab.rb`. Edits belong in the
...
puts "  https://colab.research.google.com/github/#{github_slug(options[:repo])}/blob/#{options[:branch]}/MASTER/tools/lora/#{SUBJECT}/#{target.basename}"
puts
