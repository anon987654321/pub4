...
// The server's own name table (Voice::Policy.voice_aliases). Empty when the page
// carries no policy; an unresolved name then goes to /tts as typed, and the
// server resolves it there.
const VOICE_ALIASES = window.MASTER_VOICE_POLICY?.voices || {};
...
// A reasoning model appends its working as <think>...</think> after the answer
// (ruby_llm_sender.rb does this for Nemotron 3). Read aloud, it turned a
// one-line reply into a paragraph of "We need to answer..." spoken after it.
// Cutting a closed block and an unclosed tail keeps the text prefix-stable
// while the stream grows, so the spoken-length cursor below stays valid.
...
function pushLiveStreamTts(pending, spokenLen) {
  ...
  const slice = withoutThinking(pending).slice(spokenLen);
...
function sampleDepthMapGrid(canvas, cols, rows) {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    for (let col = 0; col < cols; col++) {
      ...
      const pxu = u + ju;
      const pxv = v + jv;
      const nx = (pxu * 2 - 1) * 0.62;
      const ny = -((pxv * 2 - 1)) * 0.62;
      const nz = lum * 0.78;
...
function stepPoolAsync(pool, dtSec) {
  ...
  // serializePoolForWorker copies each array, so the main thread never reads
  // those buffers again; transferring them saves a second copy every frame.
  const payload = serializePoolForWorker(pool);
  worker.postMessage({ op: 'step', id, dt: dtSec, ctx, pool: payload }, [payload.cells, payload.decay, payload.alive]);
}
...
// The mic opens when the page loads, on the operator's ask (2026-09-15): the
// face is spoken to, and it should be listening before anyone looks for a
// control. It used to wait for a stored opt-in that only the wake word could
// write, and the mic button that once wrote it is gone, so it never opened.
//
// Permission is the browser's to remember, not this file's. Once granted, every
// later load listens without a prompt; once denied, the Permissions API says so
// and the page does not ask again. A deliberate exit ("stop listening",
// Escape) holds for the rest of the page's life and no longer outlives a reload,
// because a stored "off" was how one Escape kept the mic shut for good.
let _micOptedOut = false;
...
async function micPermissionDenied() {
  try {
...
// Safari refuses recognition.start() without a gesture even when permission is
...
if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
  ...
  recognition.onerror = (e) => {
    ...
    if (e && (e.error === 'not-allowed' || e.error === 'service-not-allowed')) {
      // A denied permission stops the retries; a refusal for want of a gesture
      // waits for one.
      if (State.voiceMode) exitVoiceMode({ reason: 'denied' });
      micPermissionDenied().then((denied) => {
        if (denied) {
          if (uiStatus) uiStatus.textContent = 'mic blocked — allow it in the browser, then reload';
          return;
        }
        setVoiceAuto(true);
...
function enterVoiceMode(opts = {}) {
  ...
  _voiceModeRearmFails = 0;
  // A deliberate entry lifts an earlier exit for the rest of the page; an auto
  // entry only reads it.
...
function exitVoiceMode(opts = {}) {
  ...
  // Any exit that isn't the device-unreliable safety bail is a user choice, and
  // it holds until the page reloads or the visitor asks again.
...
// Implicit mic: at load, and once anything already speaking has finished, slide
// straight into hands-free Voice Mode so a conversation needs no button press.
...
function handleFaceNamedEvent(event, data) {
  ...
  if (event === 'confidence') {
    ...
    if (!isNaN(c)) {
      State.confidence = c;
      // chat.js reads the turn's confidence off the body when the reply ends;
      // the felt event is not written here because it echoes the browser's own post.
      rootBody.dataset.confidence = String(c);
    }
    return true;
  }
  if (event === 'felt') {
...
async function sendMessage(text) {
  ...
  // chat_actions.js owns the one chat transport, POST /chat/message. When it
  // did not load, the turn fails the way a dropped stream does, so a broken
  // transport shows on the first message instead of hiding behind a second one.
  if (!window.MASTERChat?.startChatStream) {
    window.MASTER_LOG?.error?.("face_runtime:chat_transport_missing", new Error("MASTERChat.startChatStream is not defined"));
    onError();
    return;
  }
...
window.MASTER_FACE = {
  ...
  // uExposure's `value` as a getter the per-frame write cannot move.
