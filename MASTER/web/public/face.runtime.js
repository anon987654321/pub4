...
async function sendMessage(text) {
  ...
  if (window.MASTER_CONTAINER?.blockingSend?.(trimmed)) {
    window._chatOnUser?.(trimmed);
    window._chatOnChunk?.('master is still starting — try again in a moment.');
    window._chatOnDone?.();
    return;
  }
  if (!navigator.onLine) {
  ...
  const feltFallback = window.MASTERFeltState?.feltStateOrFallback?.(
    `${State.mood}|${State.mode}|${(State.entropy ?? 0.2).toFixed(2)}|${(State.confidence ?? 0.86).toFixed(2)}|${(State.pulse ?? 0.4).toFixed(2)}|0.00|${(State.entropy ?? 0.2).toFixed(2)}`
  );
  ...
  const sseExtensions = {
    ...
    'council:speech'(data) {
      const payload = window.MASTER_SSE?.parseSseJson?.(data, {}) || {};
  ...
  try {
    await window.MASTERChat.startChatStream({
      message: finalText,
      state: window.MASTERFeltState?.feltStateOrFallback?.(feltFallback),
      preEnhanced,
      imageToken
    }, {
      onMessage(raw) {
      ...
      onNamed(event, data) {
        window.MASTER_SSE?.dispatchNamed?.(event, data, sseExtensions);
      },
      onError() { streamFail(); }
    });
  } catch (err) {
    window.MASTER_LOG?.error?.('chat:stream', err);
    streamFail();
  }
}
