...
`visual_clusters.yml` and `mobile_web_opportunities.yml` were on that list, defended as "deliberately parallel sources in `ClusterRegistry`". `ClusterRegistry` was deleted 2026-08-03 — 109 lines with zero callers anywhere in the repo, so the defense described a reader that did not run. Both files now have no consumer at all; `SelfTest`'s `clusters` group (`lib/review/scan/self_test.rb`) is a SINGULARITY scan exemption, not a loader. Deleting them or giving them a reader is an open decision (`DEBT.md`).
