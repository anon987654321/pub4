...
class TestScanCoverage < Minitest::Test
  def self.report = @report ||= Operator::ScanCoverage.run
  ...
  def test_the_runtime_reads_the_manifest_rather_than_a_path_literal
    ...
    assert_equal Operator::ScanCoverage.manifest["roots"], Master.scan_roots,
  ...
  def test_the_fold_spine_is_inside_a_scanned_root
    roots = Operator::ScanCoverage.manifest["roots"]
  ...
  def test_every_directory_holding_ruby_is_visible_to_the_comparison
    Dir.children(Operator::ScanCoverage::MASTER).each do |child|
      next unless File.directory?(File.join(Operator::ScanCoverage::MASTER, child))
      next if Operator::ScanCoverage::NOT_SOURCE.include?(child)
      next unless Operator::ScanCoverage.ruby_count(child).positive?

      assert_includes Operator::ScanCoverage.ruby_dirs, child,
