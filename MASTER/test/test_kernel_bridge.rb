...
class KernelBridgeTest < Minitest::Test
  class ScriptedModel
  ...
  def evidence_then_done(*extra, summary:)
    [
      *extra,
      ...
      Master::Kernel::Effect.done(summary)
    ]
  end
  ...
  def test_run_returns_summary_and_streams_turns
    Dir.mktmpdir do |root|
      ...
      model = ScriptedModel.new(
        *evidence_then_done(Master::Kernel::Effect.write("note.txt", "hello\n"), summary: "wrote the note")
      )
      result = Master::Now::KernelBridge.run("write a note", root:, bus:, model:)

      assert_equal :complete, result[:reason]
      assert_equal "wrote the note", result[:summary]
      assert_equal "hello\n", File.read(File.join(root, "note.txt"))
      assert(bus.events.any? { |name, _| name == "kernel:turn" }, "expected a kernel:turn event")
    end
  end
  ...
  def test_run_string_renders_a_transcript
    Dir.mktmpdir do |root|
      ...
      out = Master::Now::KernelBridge.run_string("check", root:, model:)
  ...
  def test_empty_goal_is_refused
    assert_equal "kernel: no goal", Master::Now::KernelBridge.run_string("   ", root: Dir.pwd)
  end
end
