...
class TestEventLogRotation < Minitest::Test
  ...
  def test_appends_past_the_size_cap_rotate_instead_of_growing_forever
    Dir.mktmpdir do |dir|
      log = Master::Trace::EventLog.new(root: dir, stream: "activity")
      path = File.join(dir, "runtime", "events", "activity.jsonl")

      FileUtils.mkdir_p(File.dirname(path))
      File.write(path, "x" * (Master::Trace::EventLog::MAX_BYTES + 1))
      ...
      assert_operator File.size(path), :<, Master::Trace::EventLog::MAX_BYTES
    end
  end
