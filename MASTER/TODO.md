...
- Git archaeology dig (2026-06-15): no critical info or logic accidentally omitted. Prune ee3a56e33 (17 MDs/1142 lines: STIMULUS_ROLLOUT*, LIVE_SEARCH_STANDARD, brgen_{AUTH,CORE,DOMAIN_MATRIX}, ARCHITECTURE_NOTES, PRODUCTION_READINESS, shared/frontend LLM_SAFE+STIMULUS* — all condensed to 1 README/app + WIRING; recoveries match current surfaces/stack/activity/shared concerns in brgen/README + WIRING "Stimulus Components Baseline"/LiveSearchable + openbsd recovery). ee29827b0 (HEAD: remove unban_pf.sh) + e6d5d5712 (drop SSH overload/rate on 22): intentional (dev tmux reconnects fed <bruteforce> self-lock); manual equiv (ssh -p31415 server4; vmctl console vm23; doas pfctl -t bruteforce -T flush; ~.) fully in openbsd/README + DEPLOY/README + "For other LLMs/agents" + openbsd.sh STRICT gates/ground_truth/library_verify/scan. 5e6395ff8 thin STIMULUS_CONTROLLERS.md + .keeps (cleanup). No master.yml single source lost (config in MASTER/data/{rules,soul,patterns,...}.yml + DEPLOY/master.json + apps.yml evo; searches confirm). Other dels (dilla mp3s/clones, verify_deploy scaffold, web face dupe in 2bd175a30, handoff, priv, artifacts) bloat/scaffolding per commit msgs. Prior recover (9a2f56b94 "fix lost 'lines' var") shows vigilance. Ferrum/Scrape seeds, rules self-app waves (3cd53f4b8+), VPS light+scan gates all stuck post-add (no drops). WIRING stale pruned-MD refs cleaned in this dig. O804 snapshot terse vs full-export remains open note. (See git log ee3a56e33 ee29827b0 e6d5d5712 5e6395ff8 9a2f56b94 043f929f9 4f46ec683 3cd53f4b8; current canonicals + code + snapshots.)
