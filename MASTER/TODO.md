...
- [x] **`rake constitution` reports thousands of self-scan violations against `lib/`.** These are MASTER's
...
- [x] **Enforce `soul.yml` `self_test` in CI.** `rules.yml.self_test` defines laws-apply-to-self
...
- [x] **`SINGULARITY` self-check: no two YAMLs define the same key.** `data/` has ~72 YAML files;
...
- [x] **`Converge` subsystem is dead weight.** `lib/converge/` + `data/converge_rules.yml` +
...
- [x] **The "rules-like" data files are NOT duplication — leave them split, but document it.**
...
- [x] **Audit `data/` (72 files) and `knowledge/` (1291 files) for stale corpora.** `knowledge/`
      ...
      `knowledge/` is still referenced by `Master::Reach::SearchKnowledge` and declared in
      `data/tools.yml`; it is gitignored and skipped by fix loops/snapshots, so leave it local rather
      than moving it out from under the search tool.
- [x] **`output/` (371 files) is generated media/artifacts** — confirm it's gitignored and not
      shipped in the gem (`master.gemspec`).

## Web face (`web/`) — the recurring "tap to start does nothing" class

- [x] **Structural WebGL guard is now in place** (`chat/index.html.erb`: `getContext` returns null
      for `webgl*` until `_primerFired`). This enforces the deferred-boot contract so a stale/eager
      asset can't wedge the main thread before the tap. `web/script/probe_webgl_guard.mjs` now drives
      the browser before/after tap contract through `ci_web_probe` (`PROBE_REQUIRE_BROWSER=1` makes
      browser launch skips fatal), and `face.js` is prefetch-only so it cannot module-compile before
      the primer. Keep it — the tap has regressed
      ~5× (930a35ca5, 2faa2ffcd, a9aa0a6f6, f9b6aa57e, …).
- [x] **Broaden the asset-drift test.** `test_web_ui.rb#test_public_asset_manifest_matches_source_files`
...
- [x] **TTS end-to-end audio** depends on `edge-tts`/`espeak` on the host (see DEPLOY/TODO.md). The
...
- [x] Add a future-agent/human accommodation layer: first-contact docs, agent contract, runtime map,
      decision/debt registers, repair playbooks, examples, path ownership manifest,
      source/local-state map, web boot contract, and `bin/check*` golden-path commands.
- [x] **`nsaudit` now eager-loads and skips the kernel entrypoint** — keep it in `bin/probe all`.
- [x] Kernel spine (`kernel/`) and lib spine (`lib/`) both define a `Master::` namespace on
...
- [x] Run `rake test:web` and `rake test:integration_web` against a live Falcon in CI (they need a
      running server on `MASTER_WEB_PORT`), not just the static `test_web_ui.rb` source scans.
- [x] **Add a compact “agent handoff audit” command.** `bin/handoff` prints git state, open TODO
      items, golden check commands, dev-server ports, and browser/audio probe status.
- [x] **Make live-browser web probes first-class in CI when a browser is available.** `bin/ci`
      honors `MASTER_CI_BROWSER=1` (sets `PROBE_REQUIRE_BROWSER=1` and runs
      `web/script/probe_webgl_guard.mjs`). `.github/workflows/master-tests.yml` adds a `browser` job.
- [x] **Final TODO lifecycle gate.** `bin/todo-retire` refuses drift while unchecked items remain,
      runs MASTER/DEPLOY golden checks, and deletes both TODO files when `TODO_RETIRE_CONFIRM=1`.