...
`/run [path]`, `/scan [profile] [path]`, `/sweep [path]`, `/autoloop [N]`, `/council on|off|<path>`, `/swarm <role> <task>`, `/explain`, `/crit <file|text>`, `/ideate <prompt>`, `/topic`, `/rsi [stats]`, `/model [list|<id>]`, `/why <law|scan_rule|anti_pattern|style.key>`, `/diag [drives|breaker|rules|ring]`, `/snapshot`, `/tts`, `/brief`, `/heartbeat`, `/orders`, `/soul`, `/dmesg`. `/snapshot` publishes two GitHub gists — MASTER + DEPLOY — full tree + file bodies. `/why` resolves locally first; LLM fires only on a miss. `/diag` composes a state digest (drives, circuit-breaker, registered rule count, dmesg ring tail).
