...
module Master
  module Reach
    ...
    module MotionLoraPresets
      ...
      def training_dir(preset, root: Master::ROOT)
        File.join(root, "output", "motion_lora_training", preset.to_s)
      end
      ...
      def resolve(name)
        ...
        return { motion_lora: key, motion_lora_weight: 0.75 } if preset.nil? && key.match?(/\.safetensors\z/i)
