  ...
  module CLI
    class ResyncService
      def initialize(root:, fix_loop: nil, git: nil, bus: nil)
        ...
        @fix_loop = fix_loop
        @git = git || Io::GitOperations.new(@repo)
        @bus = bus
      end
      ...
      def execute_resync(lines)
        ...
        Master::Io::Exec.capture2e("doas", "rcctl", "restart", "master")
        sleep 2
      ...
      def bundle_install_line(dir, label)
        out, status = Master::Io::Exec.capture2e(Master::BUNDLE_BIN, "install", chdir: dir)
