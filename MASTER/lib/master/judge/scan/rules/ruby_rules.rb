...
module Master
  ...
  RuleDSL.rule :MIGRATION_ADD_REFERENCE_NO_FK,
    ...
    scan_lines(src, /add_reference(?!.*foreign_key:)/, message: "add_reference without foreign_key: true — data integrity risk")
  end
  ...
  RuleDSL.rule :MIGRATION_REMOVE_COLUMN,
    ...
    next [] unless path.include?("/db/migrate/")
    scan_lines(src, /remove_column/, message: "remove_column is destructive — confirm column is unused across all deploys")
  end
  ...
  RuleDSL.rule :KERNEL_COERCION,
    ...
    scan_lines(src, /(\w+)\s*\|\|\s*\[\](?!\s*<<)/,
      message: "foo || [] — prefer Array(foo) for nil-safe coercion")
  end
  ...
  RuleDSL.rule :HASH_FETCH,
    ...
    scan_lines(src, /\w+\[:\w+\]\s*\|\|/, message: "hash[:key] || default — prefer hash.fetch(:key, default)")
  end
