  ...
  module Voice
    class Renderer
      module GitStatus
        ...
        def git_rev
          out, _, st = Open3.capture3("git", "-C", git_root, "rev-parse", "--short", "HEAD")
        ...
        def git_branch
          out, _, st = Open3.capture3("git", "-C", git_root, "rev-parse", "--abbrev-ref", "HEAD")
          st.success? ? out.strip : nil
        rescue StandardError => e
        ...
        def git_dirty?
          out, _, st = Open3.capture3("git", "-C", git_root, "status", "--porcelain")
