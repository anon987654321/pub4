...
For a resident process that survives closing the face, run `bundle exec ruby bin/device-agent` under a Termux process supervisor. The launcher keeps the same MASTER boot, but disables the web server and TTS service in that process. The agent does not infer who owns the phone, grant Android permissions or bypass Tool::Profile / Tool::Domain boundaries.
