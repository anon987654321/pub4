...
The scanner sweeps the tree in parallel across CPUs, applies thirty-seven named rules across four scopes — Prism-AST for Ruby structure, regex for anti-patterns, registry self-checks for orphan rule files — and emits findings as data. Sweep takes those findings and rewrites the source — rubocop autocorrect first, deterministic and free; then the LLM, surgical and rate-limited; then the corruption guards reject anything that lost half its length, matched an error pattern, or failed `ruby -c`. Both `/scan` and `/sweep` default to deep depth: every rule, every cycle, sensible defaults from the Rails doctrine.
