...
**Snapshots.** Run `/snapshot` to write one source snapshot per governed tree at the repository root: `../snapshot_MASTER.md`, `../snapshot_OPENBSD.md`, `../snapshot_RAILS.md`, and `../snapshot_STUDIO.md`. Snapshots include meaningful text source only: dot files and directories, temporary/generated/dependency trees, and binary/media files are excluded. The command is deterministic apart from files that change while it runs. Generated snapshots are artifacts, not a second source of truth.
