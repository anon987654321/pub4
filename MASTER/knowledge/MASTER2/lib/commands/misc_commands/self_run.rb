      ...
      def self_run(args)
        ...
        rb_files.each do |file|
          ...
          if defined?(MASTER::Review::Fixer)
            ...
            if result.ok? && MASTER::Utils.valid_ruby?(File.read(file))
              fixed += violations.count
              puts "    + fixed"
              next
            else
              File.write(file, code) # rollback
            end
          end
          ...
          prompt = "Fix these violations in #{rel}:\n" \
                   ...
                   "Return ONLY the corrected Ruby code, no explanation, no markdown fences."
          llm_result = LLM.ask(prompt, stream: false)

          if llm_result&.ok?
            new_code = llm_result.value[:content].to_s
            # Strip markdown fences if the LLM added them despite being asked not to
        ...
        # phase 4: git commit if anything was fixed
        ...
        if system("git", "-C", root, "rev-parse", "--git-dir", out: File::NULL, err: File::NULL)
          ...
          puts status.strip.empty? ? "self: git status clean" : "self: git #{status.lines.size} uncommitted"
        end
