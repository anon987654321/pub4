...
module Master3
  ...
  def self.boot(root: Dir.pwd, argv: [])
    ...
    context  = ContextWindow.new(session:, agent:)
    ...
    stages = [
      Stages::Intake.new,
      Stages::Route.new(commands: build_commands(session:, undo:, logging:, config:, renderer:, agent:, council_stage:), agent:),
    ...
    container = {
      ...
      scanner:, bus:, breaker:, cache:, governor:, metrics:
    }
  ...
  def self.build_commands(session:, undo:, logging:, config:, renderer:, agent:, council_stage:)
    {
      ...
      "council" => ->(ctx) {
        args = ctx[:args].to_s.strip
        case args
        when "on"  then council_stage.enable!  ; "council: enabled"
        when "off" then council_stage.disable! ; "council: disabled"
        else "council is #{council_stage.instance_variable_get(:@enabled) ? "on" : "off"}"
        end
      },
      "help"    => ->(ctx) { "commands: #{%w[clear save tokens undo dmesg cost config model council help exit].map { "/#{_1}" }.join("  ")}" }
    }
  end
end
