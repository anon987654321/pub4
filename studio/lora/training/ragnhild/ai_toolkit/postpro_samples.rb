...
SKIP_PREFIX = %w[ragnhild_final_ hf_flux_].freeze
SKIP_SUFFIX = /(?:contact|grid|reel|_portrait|_cinematic|_quality_uplift|_blockbuster|_magic_hour|_postpro)\b/i

def repo_root
  ROOT.ascend.find { |path| path.join("MASTER", "tools", "postpro.rb").file? }
end
...
def main
  ...
  root = repo_root
  abort "warn: MASTER/tools/postpro.rb not found" unless root
