#!/bin/sh
# Bootstrap Johann FLUX LoRA training on a RunPod GPU pod (Linux + CUDA).
...
TOOLKIT_DIR="$WORK_ROOT/studio/lora/training/johann/ai_toolkit"
START_TRAIN=0
...
On pod:
  curl -fsSL https://raw.githubusercontent.com/anon987654321/pub4/main/studio/lora/training/johann/ai_toolkit/setup_runpod.sh | sh
  # or git clone pub4 and: ./studio/lora/training/johann/ai_toolkit/setup_runpod.sh --train
EOF
}
...
export JOHANN_DEVICE=cuda
export JOHANN_SKIP_POSTPRO=1
...
if [ "$START_TRAIN" -eq 1 ]; then
  echo "ok: starting training in tmux session johann"
  tmux kill-session -t johann 2>/dev/null || true
  tmux new-session -d -s johann \
    "cd '$TOOLKIT_DIR' && ./run_generate.sh --train 2>&1 | tee train_run.log"
  echo "ok: attach with: tmux attach -t johann"
else
  echo "ok: bootstrap done"
  echo "fix: tmux new -s johann"
