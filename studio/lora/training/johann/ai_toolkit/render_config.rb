...
BASE = File.join(ROOT, "train_johann.yaml")
...
def resolve_device
  device = ENV.fetch("JOHANN_DEVICE", "mps").downcase
  unless ALLOWED_DEVICES.include?(device)
    abort "warn: JOHANN_DEVICE must be one of: #{ALLOWED_DEVICES.join(', ')}"
  end
  device
end
...
def apply_device!(process)
  ...
  model = process["model"]
  if ENV.key?("JOHANN_LOW_VRAM")
    low = ENV["JOHANN_LOW_VRAM"] != "0"
    model["low_vram"] = low
    model["quantize"] = low
    return
  end
  ...
  when "mps"
    ...
    train["dtype"] = "fp16"
    train["lr"] = 1.0e-5 unless ENV.key?("JOHANN_LR")
    # M2 8 GB: single resolution bucket reduces VRAM churn.
    process["datasets"].first["resolution"] = [512] unless ENV.key?("JOHANN_RESOLUTIONS")
  end
end
...
def build(mode)
  ...
  requested_prompt = ENV["JOHANN_PROMPT"].to_s.strip
