...
# skip, which silently drops subdirectories; images, captions and checkpoints
# all have distinct extensions, so the notebook can sort them out by suffix.
def stage_dataset(images, checkpoints, dir)
  FileUtils.rm_rf(dir)
  FileUtils.mkdir_p(dir)
  images.each { |path| FileUtils.cp(path, dir.join(path.basename)) }
  images.each do |path|
    caption = path.sub_ext(".txt")
    FileUtils.cp(caption, dir.join(caption.basename)) if caption.file?
  end
  checkpoints.each { |path| FileUtils.cp(path, dir.join(path.basename)) }
  dir
end
...
def notebook_source(options, dataset_slug)
  toolkit = "/kaggle/tmp/pub4/studio/lora/_toolkit"
  subject_dir = "/kaggle/tmp/pub4/studio/lora/#{SUBJECT}"
  <<~PYTHON
    ...
    if not pathlib.Path("/kaggle/tmp/pub4/.git").exists():
        sh("git", "clone", "--branch", "#{options[:branch]}", "--depth", "1",
           "#{options[:repo]}", "/kaggle/tmp/pub4")
    ...
    # The mounted dataset is flat. Captions and images are the training set,
    # .safetensors are checkpoints from a previous session to resume from.
    ...
    resumed = 0
    for item in sorted(mount.iterdir()) if mount.exists() else []:
        if item.suffix == ".safetensors":
            shutil.copy2(item, weights / item.name)
            resumed += 1
        elif item.suffix.lower() in {".jpg", ".jpeg", ".png", ".webp", ".txt"}:
            shutil.copy2(item, dataset / item.name)
    print(f"ok: dataset {len(list(dataset.iterdir()))} files, checkpoints {resumed}")
...
data_dir = stage_dataset(images, checkpoints, CACHE_DIR.join("data"))
