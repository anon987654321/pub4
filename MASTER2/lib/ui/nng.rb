  ...
  module UI
    module NNGChecklist
      ...
      def compliance_score
        ...
        HEURISTICS.each_value do |heuristic|
      ...
      def report
        ...
        audit_results.each_value do |result|
      ...
      def lint_html(file_path)
        ...
        if content.include?("<style>")
          style_section = content[%r{<style>(.*?)</style>}m, 1]
          if style_section
            # Extract :root section
            style_section[/:root\s*\{[^}]*\}/m]
        ...
        issues << "Missing @media (prefers-reduced-motion) support" unless content.include?("prefers-reduced-motion")

        # Check for focus styles
        has_focus = content.include?("focus-visible") || content.include?(":focus")
        issues << "Missing focus styles (:focus or :focus-visible)" unless has_focus
      ...
      def lint_views
        ...
        results = Dir.glob(File.join(views_dir, "*.html")).map do |file|
          lint_html(file)
        end
