  ...
  module Constitution
    ...
    def rules
      ...
      @rules_cache = if File.exist?(constitution_path)
        YAML.safe_load_file(constitution_path, permitted_classes: [Symbol, Date, Time])
      else
    ...
    def axioms
      ...
      if rules["axioms"]
        ...
        @axioms_cache = File.exist?(axioms_path) ? YAML.safe_load_file(axioms_path, permitted_classes: [Symbol, Date, Time]) : []
      end
    ...
    def council
      ...
      if rules["council"]
        ...
        @council_cache = File.exist?(council_path) ? YAML.safe_load_file(council_path, permitted_classes: [Symbol, Date, Time]) : []
      end
