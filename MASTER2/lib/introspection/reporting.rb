    ...
    class Introspection
      class << self
        def print_prose_summary(results)
          ...
          # Opening
          paragraphs << if passed == total
                          "MASTER passed all #{total} self-application phases. The codebase meets its own standards."
                        elsif passed >= total - 2
                          "MASTER completed self-application with #{passed} of #{total} phases passing. A few areas need attention."
                        else
                          "Self-application found gaps in #{total - passed} of #{total} phases. Significant work remains."
                        end

          # Static analysis and structure
          issues_summary = []
          issues_summary << "#{static[:issues] || 0} static analysis issues" if static[:issues].to_i > 0
          if consistency[:issues]&.size.to_i > 0
            issues_summary << "#{consistency[:issues]&.size || 0} consistency issues"
          end
          if enforcement[:violations]&.size.to_i > 0
            issues_summary << "#{enforcement[:violations]&.size || 0} axiom violations"
          end
