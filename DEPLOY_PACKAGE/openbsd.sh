...
http protocol "https" {
  ...
  # Security headers (OWASP + securityheaders.com best practices)
  ...
  # ActionCable/StimulusReflex WebSocket support
  http websockets
...
EOF
  ...
  # Puma/Falcon configuration via Rails

  doas -u "$app" cat > "$app_dir/config/puma.rb" << PUMA
...
PUMA
  ...
  # rc.d service script - use bin/rails server (Falcon via Rack)
...
# rc.d script for ${app} Rails app

# Uses bin/rails server (Falcon via Rack adapter)
...
. /etc/rc.d/rc.subr
pexp="ruby33.*bin/rails server.*-p $port"
