...
TOOLKIT_DIR="$WORK_ROOT/lora/training/ragnhild/ai_toolkit"
START_TRAIN=0
...
On pod:
  curl -fsSL https://raw.githubusercontent.com/anon987654321/pub4/main/lora/training/ragnhild/ai_toolkit/setup_runpod.sh | sh
  # or git clone pub4 and: ./lora/training/ragnhild/ai_toolkit/setup_runpod.sh --train
EOF
}
...
export RAGNHILD_DEVICE=cuda
export RAGNHILD_SKIP_POSTPRO=1
...
if [ "$START_TRAIN" -eq 1 ]; then
  echo "ok: starting training in tmux session ragnhild"
  tmux kill-session -t ragnhild 2>/dev/null || true
  tmux new-session -d -s ragnhild \
    "cd '$TOOLKIT_DIR' && ./run_generate.sh --train 2>&1 | tee train_run.log"
  echo "ok: attach with: tmux attach -t ragnhild"
else
  echo "ok: bootstrap done"
  echo "fix: tmux new -s ragnhild"
