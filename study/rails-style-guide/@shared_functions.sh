...
PATH="/home/dev/.local/share/gem/ruby/3.4/bin:${PATH:-/usr/local/bin:/usr/bin:/bin}"
...
create_rails_app() {
  local app_dir=$1
  mkdir -p "${app_dir:h}"
  $_PRIV chown "${USER}:${USER}" "$app_dir"
  mkdir -p "${app_dir}/.bundle"
  cat > "${app_dir}/.bundle/config" << 'BCFG'
---
...
daemon_flags="exec env RAILS_ENV=production SECRET_KEY_BASE_DUMMY=1 falcon serve --count 1 --bind http://127.0.0.1:${port}"
daemon_user="${user}"
...
EOS
  ...
  log_ok "rc.d/${svc} installed on port ${port}"
}
